...
from depth_network_2 import DepthNetwork
...
if __name__ == "__main__":
    writer = SummaryWriter('runs/depth_exper_img_1')
    ...
    for epoch in range(nb_epochs + 1):
        ...
        for i, samples in enumerate(dataloader):
            ...
            if (i + 1) % 900 == 0 and i != 0:  # every 100 mini-batches
                ...
                np_gt = np.array(gt.cpu())
                print(np.unique(np_gt))
                np_gt = np_gt.squeeze(axis=0)
                print(np_gt.shape)
                gt_dict = {"gt":np_gt}

                scipy.io.savemat("view_gt.mat",gt_dict)
