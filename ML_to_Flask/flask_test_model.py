...
def img_view():
    app.logger.info('Processing default request')
    return send_file("resize.png", mimetype='image/png')
...
def test():
    ...
    if request.method == "POST":
        ...
        ori_image = cv2.imread("test.png", cv2.IMREAD_COLOR)
        cv2.imwrite("ori_image.png", ori_image)

        print("-------------", flush=True)
        our_image = cv2.resize(dsize=(720,960), src=ori_image, interpolation=cv2.INTER_AREA)
        #our_image = ori_image
        cv2.imwrite("resize.png", our_image)
        ...
        outputs = outputs.squeeze()
        print(outputs.shape)
        output_img = outputs.cpu().detach().numpy()
        #backtorgb = cv2.cvtColor(output_img, cv2.COLOR_GRAY2RGB)
...
if __name__ == '__main__':
    ...
    MY_MODEL_DIR = os.path.join(ROOT_DIR+"/model/", "mask_rcnn_arms_0056.h5")
